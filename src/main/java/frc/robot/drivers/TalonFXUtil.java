...
public class TalonFXUtil {
    ...
    public synchronized static void setSupplyCurrentLimit(final LazyTalonFX falcon, int amps) {
        ...
        PhoenixUtil.checkError(falcon.configSupplyCurrentLimit(supplyCurrent),
    ...
    public synchronized static void setStatorCurrentLimit(final LazyTalonFX falcon, int amps) {
        ...
        PhoenixUtil.checkError(falcon.configStatorCurrentLimit(statorCurrent),
    ...
    public static synchronized void checkMotorFaults(final LazyTalonFX falcon) {
        ...
        PhoenixUtil.checkError(falcon.getStickyFaults(faults),
        ...
        if (hasFaults) {
            PhoenixUtil.checkError(falcon.clearStickyFaults(),
                    falcon.getName() + " failed to clear sticky faults", true);
        }
    }

    /**
     * checks if slave motor has any stick faults: under voltage and reset during
     * enabled
    ...
    public static synchronized void checkSlaveFaults(final LazyTalonFX slaveFalcon, int master_ID) {
        StickyFaults faults = new StickyFaults();
        PhoenixUtil.checkError(slaveFalcon.getStickyFaults(faults),
        ...
        if (hasFaults) {
            PhoenixUtil.checkError(slaveFalcon.clearStickyFaults(),
    ...
    /**
     * checks weather the talon has any of the sticky faults: under voltage, reset
     * during enabled, sensor out of phase, remote loss of signa, and sensor
     * overflow
     * If it does have any faults, it will log them and then clear it.
     * 
