...
public class TalonSRXUtil {
    ...
    public static void setCurrentLimit(final LazyTalonSRX talon, int amps) {
        PhoenixUtil.checkError(talon.configPeakCurrentLimit(amps, Constants.kTimeOutMs),
                "Failed to set peak current limit for " + talon.getName(), true);
        PhoenixUtil.checkError(talon.configContinuousCurrentLimit(amps, Constants.kTimeOutMs),
                "Failed to set continious current limit for " + talon.getName(), true);
        PhoenixUtil.checkError(talon.configPeakCurrentDuration(0, Constants.kTimeOutMs),
    ...
    public static void testFaults(final LazyTalonSRX talon) {
        ...
        PhoenixUtil.checkError(talon.getStickyFaults(faults), "FAILED TO RETRIEVE FAULTS FROM " + talon.getName(),
                true);
        ...
        if (hasFaults) {
            PhoenixUtil.checkError(talon.clearStickyFaults(), "FAILED TO RESET FAULTS FOR " + talon.getName(), true);
        }
    ...
    public static void testSlaveFaults(LazyTalonSRX slaveTalon, LazyTalonSRX masterTalon) {
        StickyFaults faults = new StickyFaults();
        PhoenixUtil.checkError(slaveTalon.getStickyFaults(faults), "FAILED TO RETRIEVE FAULTS FROM " +
        ...
        if (hasFaults) {
            PhoenixUtil.checkError(slaveTalon.clearStickyFaults(), "FAILED TO CLEAR FAULTS FOR "
                    + slaveTalon.getName(), true);
        }
    }
